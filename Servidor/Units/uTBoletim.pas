...
type
  ...
  public
   ...
   procedure lancIndividual(codLei, selo:string; emolumento, tipoSelo, boletim, fPag, protocolo:Integer; valorTransacao, vlrEmolumento, vlrFerj, vlrFerc: Double);
...
begin
    sql := TSQLDataSet.Create(nil);
    sql.SQLConnection := DM.SQLConnection;
    sql.CommandText := 'UPDATE BOLETIM SET STATUS = 1 WHERE CODIGO =:CODIGO';
    sql.Params[0].AsInteger := codigo;
    sql.ExecSQL;
    sql.Free;
end;
...
procedure TBoletim.lancIndividual(codLei, selo: string; emolumento, tipoSelo, boletim, fPag, protocolo:Integer; valorTransacao, vlrEmolumento, vlrFerj, vlrFerc: Double);
var
  sql: TSQLDataSet;
begin
    sql := TSQLDataSet.Create(nil);
    sql.SQLConnection := DM.SQLConnection;
    sql.CommandText := 'INSERT INTO LANCBOLEINDIVIDUAL (CODIGO,CODLEI,EMOLUMENTO,TIPOSELO,VALORTRANSACAO,VLREMOLUMENTOS,SELO,VLRFERJ,VLRFERC,DATAATO,BOLETIM,FORMAPAGAMENTO,NUMFOLHA,NUMLIVRO,NUMATO) VALUES (0,:A,:B,:C,:D,:E,:F,:G,:H,CURRENT_DATE,:I,:J,:K,1,:L)';
    ...
    sql.Params[9].AsInteger := fPag;
    sql.Params[10].AsString := IntToStr(protocolo);
    sql.Params[11].AsString := IntToStr(protocolo);
    sql.ExecSQL;
    sql.Free;
end;
