...
begin
if dbPagoFerc.Checked then
  begin
    dbNumBoletoFerc.Enabled := True;
    dbDigVerFerc.Enabled := True;
    dbNumBoletoFerc.Color := clWhite;
    dbDigVerFerc.Color := clWhite;
  end
else
  begin
    dbNumBoletoFerc.Enabled := False;
    dbDigVerFerc.Enabled := False;
    dbNumBoletoFerc.Color := clSkyBlue;
    dbDigVerFerc.Color := clSkyBlue;
  end;
end;
...
if dbPagoFerj.Checked then
  begin
    ...
    dbNumBoletoFerj.Color := clWhite;
    dbDigVerifFerj.Color := clWhite;
  end
else
  begin
    dbNumBoletoFerj.Enabled := False;
    dbDigVerifFerj.Enabled := False;
    dbNumBoletoFerj.Color := clSkyBlue;
    dbDigVerifFerj.Color := clSkyBlue;
  end;
end;
...
begin
 ...
 cdsCadastroPAGOFERJ.AsInteger := 1;
 cdsCadastroPAGOFERC.AsInteger := 1;
end;

procedure TFCadBoletim.dbFinalExit(Sender: TObject);
begin
  inherited;
cdsCadastroNUMREMESSA.AsString := IntToStr(datas.anoAtual)+IntToStr(datas.getSemanaDoAno(cdsCadastroDATAINI.AsDateTime));
end;
