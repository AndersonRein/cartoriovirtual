...
type
  ...
  public
    ...
    function getVlrTipoVenda(caixa, fPagamento: Integer): Double;
...
function TFCaixa.getVlrTipoVenda(caixa, fPagamento: Integer): Double;
var
  sql: TSQLDataSet;
begin
  sql := TSQLDataSet.Create(nil);
  sql.SQLConnection := DM.SQLConnection;
  sql.CommandText :=
    'SELECT COALESCE(SUM(FATURAMENTO_SB.VALOR),0) AS TOTAL FROM FATURAMENTO_SB INNER JOIN VENDAS_SB ON (VENDAS_SB.CODIGO = FATURAMENTO_SB.VENDA) WHERE FATURAMENTO_SB.CAIXA =:CAIXA AND FATURAMENTO_SB.FPAGAMENTO = :FPAGAMENTO AND VENDAS_SB.STATUS = 0';
  sql.Params[0].AsInteger := caixa;
