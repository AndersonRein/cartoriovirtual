...
begin
  ...
  I := 1;
  endeAquivo := endereco+serventia+numRemessa+'.rem';
  ...
  while not cds.Eof do
    begin
      ...
      Writeln(arq,FormatFloat('00000',cds.FieldByName('QUANTIDADE').AsInteger)+delimit+formatarValores(cds.FieldByName('VLRFERJ').AsFloat)+delimit+cds.FieldByName('CODTIPOSELO').AsString+delimit+formatarValores(cds.FieldByName('EMOLUNIDADE').AsFloat)+delimit+formatarValores(cds.FieldByName('VLRFERC').AsFloat)+delimit);
      Inc(I);
      cds.Next;
    end;
